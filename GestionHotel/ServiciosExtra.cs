...
{
    public partial class ServiciosExtra : Form
    {
        ...
        private void buttonVolverInicio_Click(object sender, EventArgs e)
        {
            ...
            var volver = MessageBox.Show("¿Quiere volver a la ventana principal?", "Cerrar servicios extra", MessageBoxButtons.OKCancel);
        ...
        private void EstadoControlesInicioApp()
        {
            ...
            //Campos
            idServiciosExtraLabel1.Enabled = false;
            parkingCheckBox.Enabled = false;
            excursionesComboBox.Enabled = false;
            guarderiaRadioButton.Enabled = false;
            guarderiaRadioButton1.Enabled = false;
            alquilerCochesCheckBox.Enabled = false;
            trasladoAeropuertoComboBox.Enabled = false;
            spaComboBox.Enabled = false;
            gimnasioComboBox.Enabled = false;
            ...
            if (serviciosExtraBindingSource.Count < 1)
            {
            ...
            else if (serviciosExtraBindingSource.Count == 1)
            {
                toolStripButtonSiguiente.Enabled = false;
                toolStripButtonFinal.Enabled = false;
                toolStripButtonAnterior.Enabled = false;
        ...
        {
            serviciosExtraBindingSource.MoveFirst();
            toolStripButtonInicio.Enabled = false;
            toolStripButtonAnterior.Enabled = false;
        ...
        {
            serviciosExtraBindingSource.MovePrevious();
            if (serviciosExtraBindingSource.Position <= 0)
            {
                toolStripButtonAnterior.Enabled = false;
                toolStripButtonInicio.Enabled = false;
            }
        ...
        {
            serviciosExtraBindingSource.MoveNext();
            if (serviciosExtraBindingSource.Count <= 0 || serviciosExtraBindingSource.Position + 1 == serviciosExtraBindingSource.Count)
            {
                toolStripButtonSiguiente.Enabled = false;
                toolStripButtonFinal.Enabled = false;
            }
        ...
        {
            serviciosExtraBindingSource.MoveLast();
            toolStripButtonSiguiente.Enabled = false;
            toolStripButtonFinal.Enabled = false;
        ...
        private void RefrescarToolstripLabelServicio()
        {
            this.toolstripLabelContadorIncidencias.Text = $"Servicio {serviciosExtraBindingSource.Position + 1} de {serviciosExtraBindingSource.Count}";
        }

        private void toolStripButtonAnadir_Click(object sender, EventArgs e)
        {
            toolStripStatusLabel1.Text = "Añadir servicio extra";
            toolStripButtonSiguiente.Enabled = false;
            toolStripButtonFinal.Enabled = false;

            serviciosExtraBindingSource.AddNew();
        ...
        private void HabilitarControlesEnAnadir()
        {
            ...
            //Campos
            idServiciosExtraLabel1.Enabled = true;
            parkingCheckBox.Enabled = true;
            excursionesComboBox.Enabled = true;
            guarderiaRadioButton.Enabled = true;
            guarderiaRadioButton1.Enabled = true;
            alquilerCochesCheckBox.Enabled = true;
            trasladoAeropuertoComboBox.Enabled = true;
            spaComboBox.Enabled = true;
            gimnasioComboBox.Enabled = true;

            //ComboBox por defecto a una opción
            excursionesComboBox.SelectedIndex = 0;
            trasladoAeropuertoComboBox.SelectedIndex = 0;
            spaComboBox.SelectedIndex = 0;
            gimnasioComboBox.SelectedIndex = 0;
        }
        ...
        {
            toolStripStatusLabel1.Text = "Eliminar servicio extra";

            var resultado = MessageBox.Show("¿Está seguro que desea eliminar el servicio?", "Confirmación eliminar servicio", MessageBoxButtons.OKCancel);

            if (resultado == DialogResult.OK)
            {
                if (serviciosExtraBindingSource.Count <= 0)
                {
                    MessageBox.Show("No se puede eliminar el servicio porque no existe en la base de datos ", "Error en la eliminación de un servicio", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    serviciosExtraBindingSource.RemoveCurrent();
                }
                ...
                if (serviciosExtraBindingSource.Position + 1 == serviciosExtraBindingSource.Count)
                {
                    toolStripButtonSiguiente.Enabled = false;
                    toolStripButtonFinal.Enabled = false;
                }
        ...
        {
            toolStripStatusLabel1.Text = "Editar servicio extra";
        ...
        private void toolStripButtonGuardar_Click(object sender, EventArgs e)
        {
            ...
            this.serviciosExtraTableAdapter.Update(this.hotelDataSet); //Guarda en la base de datos
        ...
        private void EstadoControlesGuardar()
        {
            ...
            if (serviciosExtraBindingSource.Position + 1 > 1)
            {
                toolStripButtonAnterior.Enabled = true;
                toolStripButtonInicio.Enabled = true;
            ...
            if (serviciosExtraBindingSource.Position + 1 == serviciosExtraBindingSource.Count && serviciosExtraBindingSource.Count > 1)
            {
                toolStripButtonAnterior.Enabled = true;
                toolStripButtonInicio.Enabled = true;
                toolStripButtonSiguiente.Enabled = false;
            ...
            if (serviciosExtraBindingSource.Count > 2 && serviciosExtraBindingSource.Position + 1 != serviciosExtraBindingSource.Count && serviciosExtraBindingSource.Position + 1 != 1)
            {
                toolStripButtonAnterior.Enabled = true;
                toolStripButtonInicio.Enabled = true;
                toolStripButtonSiguiente.Enabled = true;
            ...
            if (serviciosExtraBindingSource.Position + 1 == 1 && serviciosExtraBindingSource.Count > 1)
            {
                toolStripButtonAnterior.Enabled = false;
                toolStripButtonInicio.Enabled = false;
                toolStripButtonSiguiente.Enabled = true;
        ...
        {
            toolStripStatusLabel1.Text = "Buscar servicios extra"; 
            try
            {
                ...
                else
                {
                    ...
                    {
                        if (serviciosExtraBindingSource.Find("IdServiciosExtra", toolStripTextBoxBuscar.Text) == -1)
                        {
                        ...
                        {
                            serviciosExtraBindingSource.Position = serviciosExtraBindingSource.Find("IdServiciosExtra", toolStripTextBoxBuscar.Text);
                        }
                    }
        ...
        private void buttonAceptar_Click(object sender, EventArgs e)
        {
            if (ComprobarDatosIntroducidos())
            {
                ...
                serviciosExtraBindingSource.EndEdit(); //Lo guarda en memoria
        ...
        private void buttonCancelar_Click(object sender, EventArgs e)
        {
            ...
            if (resultado == DialogResult.OK)
            {
                serviciosExtraBindingSource.CancelEdit();
        ...
        private void toolStripButtonImprimir_Click(object sender, EventArgs e)
        {
            ...
            string id = idServiciosExtraLabel1.Text;
            string parking = parkingCheckBox.Text;
            string excursiones = excursionesComboBox.Text;
            string guarderia = guarderiaRadioButton.Text;
            string guarderia2 = guarderiaRadioButton1.Text;
            string transladoAeropuerto = trasladoAeropuertoComboBox.Text;
            string spa = spaComboBox.Text;
            string gimnasio = gimnasioComboBox.Text;
            ...
            if (printDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    e1.Graphics.DrawString("Id Servicios Extra: " + id, new Font("Times New Roman", 12), new SolidBrush(Color.Black), new RectangleF(50, 100, printDocument1.DefaultPageSettings.PrintableArea.Width, printDocument1.DefaultPageSettings.PrintableArea.Height));
                    e1.Graphics.DrawString("Parking: " + parking, new Font("Times New Roman", 12), new SolidBrush(Color.Black), new RectangleF(50, 125, printDocument1.DefaultPageSettings.PrintableArea.Width, printDocument1.DefaultPageSettings.PrintableArea.Height));
                    e1.Graphics.DrawString("Excursiones: " + excursiones, new Font("Times New Roman", 12), new SolidBrush(Color.Black), new RectangleF(50, 150, printDocument1.DefaultPageSettings.PrintableArea.Width, printDocument1.DefaultPageSettings.PrintableArea.Height));
                    e1.Graphics.DrawString("Guarderia: " + guarderia, new Font("Times New Roman", 12), new SolidBrush(Color.Black), new RectangleF(50, 175, printDocument1.DefaultPageSettings.PrintableArea.Width, printDocument1.DefaultPageSettings.PrintableArea.Height));
                    e1.Graphics.DrawString("Guarderia: " + guarderia2, new Font("Times New Roman", 12), new SolidBrush(Color.Black), new RectangleF(50, 200, printDocument1.DefaultPageSettings.PrintableArea.Width, printDocument1.DefaultPageSettings.PrintableArea.Height));
                    e1.Graphics.DrawString("Translado al aeropuerto: " + transladoAeropuerto, new Font("Times New Roman", 12), new SolidBrush(Color.Black), new RectangleF(50, 225, printDocument1.DefaultPageSettings.PrintableArea.Width, printDocument1.DefaultPageSettings.PrintableArea.Height));
                    e1.Graphics.DrawString("Spa: " + spa, new Font("Times New Roman", 12), new SolidBrush(Color.Black), new RectangleF(50, 250, printDocument1.DefaultPageSettings.PrintableArea.Width, printDocument1.DefaultPageSettings.PrintableArea.Height));
                    e1.Graphics.DrawString("Gimnasio: " + gimnasio, new Font("Times New Roman", 12), new SolidBrush(Color.Black), new RectangleF(50, 275, printDocument1.DefaultPageSettings.PrintableArea.Width, printDocument1.DefaultPageSettings.PrintableArea.Height));
        ...
        {
            toolStripStatusLabel1.Text = "Informe servicios extra";
            Boolean abierto = false;

            //comprobamos que no esta abierto el formulario;
            foreach (Form frm in Application.OpenForms)
            {
                if (frm.GetType() == typeof(InformeServicios))
                {
            ...
            if (!abierto)
            {
                InformeServicios informeServicios = new InformeServicios();
                informeServicios.ShowDialog();
            }
        }
        ...
        private void ServiciosExtra_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (!(hotelDataSet.GetChanges() is null))
            {
                ...
                {
                    this.serviciosExtraBindingSource.CancelEdit();
